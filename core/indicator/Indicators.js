...
const { setConfig, MACD: _MACD, RSI: _RSI, ATR: _ATR, BollingerBands, EMA: _EMA, SMA: _SMA, OBV: _OBV, MFI: _MFI, KST: _KST, bullish, WilliamsR: _WilliamsR, ADL: _ADL, ADX: _ADX, AwesomeOscillator: _AwesomeOscillator, CCI: _CCI, ForceIndex: _ForceIndex, PSAR: _PSAR, ROC: _ROC, Stochastic: _Stochastic, TRIX: _TRIX, VWAP: _VWAP, VolumeProfile: _VolumeProfile, WMA, WEMA: _WEMA, IchimokuCloud: _IchimokuCloud, abandonedbaby, bearishengulfingpattern, bullishengulfingpattern, darkcloudcover, downsidetasukigap, doji, dragonflydoji, gravestonedoji, bullishmarubozu, bearishmarubozu, eveningdojistar, eveningstar, bearishharami, piercingline, bullishspinningtop, bearishspinningtop, morningdojistar, morningstar, threeblackcrows, threewhitesoldiers, bullishhammerstick, bearishhammerstick, bullishinvertedhammerstick, bearishinvertedhammerstick, hammerpattern, hammerpatternunconfirmed, hangingman, hangingmanunconfirmed, shootingstar, shootingstarunconfirmed, tweezertop, tweezerbottom, CrossUp: _CrossUp, CrossDown: _CrossDown, crossOver: _crossOver, Highest: _Highest, Lowest: _Lowest } = pkg;
setConfig('precision', 10);

import { getLastLookbackHigh, getLastLookbackLow, woodies, floorPivots } from "../cryptoDogTools.js";
...
let KST     =  _KST;
let Bullish =  bullish;
let WilliamsR = _WilliamsR;
let ADL = _ADL;
let ADX = _ADX;
let AwesomeOscillator = _AwesomeOscillator;
let CCI = _CCI;
let ForceIndex = _ForceIndex;
let PSAR = _PSAR;
let ROC = _ROC;
let Stochastic = _Stochastic;
let TRIX = _TRIX;
let VWAP = _VWAP;
let VP = _VolumeProfile;
let WP = WMA;
let WEMA = _WEMA;
let IchimokuCloud  = _IchimokuCloud;
import SuperTrend from 'node-super-trend';
...
class MultiDivergenceDetector extends CustomIndicator {
    ...
    detectRSIDivergence(highBuffer, lowBuffer) {
      ...
      calculate(){
        ...
        let lowBuffer = []
        let buffer = [];
        highBuffer = getLastLookbackHigh(highs,this.lookback) ;
        lowBuffer =getLastLookbackLow(lows,this.lookback); 
...
class SuperTrendIndicator {
    ...
    static getData(o, h, l, c, v, args, candles) {
        ...
        const multiplier = args.multiplier || 3;
        ...
        let atrValues = atr.getResult();
        
        // Calculate HL/2 (High + Low) / 2
        const hl2 = [];
        ...
        // ATR starts from index (period-1), so we need to align arrays
        const offset = h.length - atrValues.length;
        ...
        for (let i = 0; i < h.length; i++) {
            if (i < offset) {
                ...
                basicUpperBand.push(hl2[i] + multiplier * atrValues[atrIndex]);
                basicLowerBand.push(hl2[i] - multiplier * atrValues[atrIndex]);
            }
        }
        ...
        for (let i = 0; i < h.length; i++) {
            if (i === 0 || isNaN(basicUpperBand[i])) {
                finalUpperBand.push(basicUpperBand[i]);
                finalLowerBand.push(basicLowerBand[i]);
            } else {
                // Final Upper Band
                if (isNaN(basicUpperBand[i - 1])) {
                    finalUpperBand.push(basicUpperBand[i]);
                } else {
                    finalUpperBand.push(
                        basicUpperBand[i] < finalUpperBand[i - 1] || c[i - 1] > finalUpperBand[i - 1]
                            ? basicUpperBand[i]
                            : finalUpperBand[i - 1]
                    );
                }
                ...
                // Final Lower Band
                if (isNaN(basicLowerBand[i - 1])) {
                    finalLowerBand.push(basicLowerBand[i]);
                } else {
                    finalLowerBand.push(
                        basicLowerBand[i] > finalLowerBand[i - 1] || c[i - 1] < finalLowerBand[i - 1]
                            ? basicLowerBand[i]
                            : finalLowerBand[i - 1]
                    );
                }
        ...
        for (let i = 0; i < h.length; i++) {
            if (isNaN(finalUpperBand[i]) || isNaN(finalLowerBand[i])) {
                superTrend.push({ trend: null, value: NaN });
                continue;
            }
            
            if (i === 0 || trend === null) {
                // Initial trend determination
                if (c[i] <= finalUpperBand[i]) {
                    trend = 'short';
                    superTrend.push({ trend: 'short', value: finalUpperBand[i] });
                } else {
                    trend = 'long';
                    superTrend.push({ trend: 'long', value: finalLowerBand[i] });
                }
            } else {
                // Trend continuation or reversal
                const prevTrend = superTrend[i - 1].trend;
                
                if (prevTrend === 'long') {
                    if (c[i] <= finalLowerBand[i]) {
                        trend = 'short';
                        superTrend.push({ trend: 'short', value: finalUpperBand[i] });
                    } else {
                        trend = 'long';
...
class Woodies {
     ...
     */
    static getData(o,h,l,c,v, args, candles){
      return woodies(o,h,l,c,v, args, candles)
    }
}
/**
 *  Class FloorPivots
...
class FloorPivots {
     ...
     */
    static getData(o,h,l,c,v, args, candles){
        return floorPivots(o,h,l,c,v, args, candles)
    }
}
...
class EMAIndicator{
     ...
     * */
    static getData(o,h,l,c,v, args, candles){
        let ema = new  EMA({period : args.period || 200, values  : c});
...
class Ema3Indicator{
     ...
     * */
    static getData(o,h,l,c,v, args,candles ){
        let EMA1 = new  EMA({period : args.period1 || 20, values  : c});
        let EMA2 = new  EMA({period : args.period2 || 50, values : c});
        let EMA3 = new  EMA({period : args.period3 || 200, values : c});
        let object = {};
        let tmpBuffer = EMA1.getResult();
...
class IndicatorList{
    ...
    static getIndicator (key) {
        let map = {
            ...
            "SuperTrendIndicator":SuperTrendIndicator.getData,
            ...
            "MfiIndicator":MfiIndicator.getData,
            "ObvIndicator":ObvIndicator.getData,
            "Ema4Indicator":Ema4Indicator.getData,
            "Ema3Indicator":Ema3Indicator.getData,
            "Ema10And20":Ema10And20.getData,
            "Sma3Indicator":Sma3Indicator.getData,
            "AdlIndicator":AdlIndicator.getData,
            "AdxIndicator":AdxIndicator.getData,
            "AwesomeOscillatorIndicator":AwesomeOscillatorIndicator.getData,
            "CciIndicator":CciIndicator.getData,
            "StochasticIndicator":StochasticIndicator.getData,
            "IchimokuCloudIndicator":IchimokuCloudIndicator.getData,
            "WildersSmoothingWeighteMovingAvg":WildersSmoothingWeighteMovingAvg.getData,
            "WeighteMovingAvg":WeighteMovingAvg.getData,
            "VolumeProfile":VolumeProfile.getData,
            "VolumeWeightedAvgPrice":VolumeWeightedAvgPrice.getData,
            "TrixIndicator":TrixIndicator/getLastLookbackHigh,
            "ForceIndexIndiactor":ForceIndexIndiactor.getData,
            "RocIndicator":RocIndicator.getData,
            "PsarIndicator":PsarIndicator.getData,
...
class PatternRecognitionIndicator{
    ...
    static BearishEngulfingPattern(o,h,l,c){
        let pattern = bearishengulfingpattern;
        return pattern({open:o,high:h,low:l,close:c})
    }
    /**
     *
    ...
    static BullishEngulfingPattern(o,h,l,c){
        let pattern = bullishengulfingpattern;
        return pattern({open:o,high:h,low:l,close:c})
    }
    /**
     *
    ...
    static BullishSpinningTop(o,h,l,c){
        let pattern = bullishspinningtop;
        return pattern({open:o,high:h,low:l,close:c})
    }
    /**
     *
    ...
    static BearishSpinningTop(o,h,l,c){
        let pattern = bearishspinningtop;
        return pattern({open:o,high:h,low:l,close:c})
    }
    /**
     *
    ...
    static ThreeWhiteSoldiers(o,h,l,c){
        let pattern = threewhitesoldiers;
        return pattern({open:o,high:h,low:l,close:c})
    }
    /**
     *
    ...
    static BullishHammer(o,h,l,c){
        let pattern = bullishhammerstick;
        return pattern({open:o,high:h,low:l,close:c})
    }
    /**
     *
    ...
    static BearishHammer(o,h,l,c){
        let pattern = bearishhammerstick;
        return pattern({open:o,high:h,low:l,close:c})
    }
    /**
     *
    ...
    static BullishInvertedHammer(o,h,l,c){
        let pattern = bullishinvertedhammerstick;
        return pattern({open:o,high:h,low:l,close:c})
    }
    /**
     *
    ...
    static BearishInvertedHammer(o,h,l,c){
        let pattern = bearishinvertedhammerstick;
        return pattern({open:o,high:h,low:l,close:c})
    }
    /**
     *
    ...
    static HammerPatternUnconfirmed(o,h,l,c){
        let pattern = hammerpatternunconfirmed;
        return pattern({open:o,high:h,low:l,close:c})
    }
    /**
     *
    ...
    static HangingManUnconfirmed(o,h,l,c){
        let pattern = hangingmanunconfirmed;
        return pattern({open:o,high:h,low:l,close:c})
    }
    /**
     *
    ...
    static ShootingStarUnconfirmed(o,h,l,c){
        let pattern = shootingstarunconfirmed;
        let set = [
